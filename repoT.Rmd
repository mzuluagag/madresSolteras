...
<p style = "font-size:20px" align="justify">
Nota: En la encuesta de el año 2016, no se incluyó la variable(o alguna sustituta) "¿Siente que vale la pena lo que hace?" por lo tanto, se recalculó un modelo lineal sin esa variable y se obtuvo un $R^2$ de 42%. Luego de esto, al filtrar los datos, se notó que a comparación de 2017, los datos estaban en escala de 1 a 4 y algunos estaban cómo que el menor número representaba la mejor calidad de vida, entonces se aplicaron transformaciones para tomar todos los datos en la escala y orden del modelo de 2017.
</p>

<p style = "font-size:20px" align="justify">
Aparentemente se puede visualizar que los datos tienen forma de campana. Los datos están centrados alrededor de la media que es 5.34 aproximádamente, con varianza 1.04; por lo tanto se puede entender y ver que la mayoría de los datos están alrededor de la media entre los valores 4 y 7 aproximádamente.
</p>

<h3><b>Construcción y funcionamiento: Red de composición(hogar)</b></h3>
<br>
<p style = "font-size:20px" align="justify">
Para construir las redes, se usó específicamente la tabla de "Características y composición del hogar", ya que esta posee variables tales cómo:"¿Vive con la madre?","No. Orden de la madre","¿Vive con el padre?","No. Orden de el padre", "¿Vive con el/la cónyuge?","No. Orden de el/la cónyuge", "relación con el jefe de hogar", etc. A partir de un sistema de condicionales para crear un vector que indicara las coordenadas entre los nodos(miembros del hogar),otro vector que indicara el tipo de relación que poseía con el jefe de hogar y otro vector que almacenara la relación con el nodo inmediatamente anterior relacionado, lo cuál permite saber en toda la red saber que parentesco tienen todos los nodos con el jefes de hogar. También a partir de diferentes colores para las relaciones, se puede saber quién es la madre, padre y/o cónyuge de cada nodo y averiguar de una manera casi completa la composición del hogar(no se puede deducir, en algunos casos, el género de algunos nodos).
</p>
<br>
<p style = "font-size:20px" align="justify">
